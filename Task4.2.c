...
*/
const int getCheck(const int* arr, const size_t rows);
...
void check_int(const int input, const int min, const int max)
{
    ...
    {
        fprintf(stderr, "Error Ошибка ввода, число должно лежать в указанном промежутке\n");
        exit(1);
    }
}
...
void def_task_two(const int* arr, const size_t rows)
{
    ...
    const size_t newsize = rows + getCheck(arr, rows);
